import numpy as np #lol this is going to annoy people
...
Ib, Id = np.transpose(((F*np.conj(F)).real), (2,3,0,1)).reshape(2,xsiz,ysiz)
...
BROKEN STUFF MAY NEED AT SOME POINT???

top_mat = np.tile(top_vec, (xsiz, zsiz+1, 1)) # 3-tensor (xsiz, zsiz+1, ysiz)
#print(top_mat)
h_mat = top_mat.astype(int)
print(h_mat)
m_mat = top_mat - h_mat
#print(m_mat)


sxzy = np.tile(sxz.reshape(xsiz, zsiz+1, 1), (1, 1, ysiz))
print(sxzy)


sxzyh = np.roll(sxzy, h, axis=1)


#slocal = s + (1-m)*sxz + m*sxz
...
'''
    #sxz is xz matrix
    #do all rows parallel and iterate along them
    
    
    #slocal should be an xy matrix
    #for each z, extract the vector along x associated
    
    sxz_x_vec = np.transpose(sxz)[z]
    
    #now need 2 matrices of these vectors
    #accounting for the y offsets given by h
    
    
    sxz_xymatT = np.zeros((ysiz, xsiz))
    sxz_xymat1T = np.zeros((ysiz, xsiz))
    
    #fine I'll use a for loop just this once to get this working
    #NEEDS REMOVING LATER
    for y in range(ysiz):
        sxz_xymatT[y] = np.transpose(sxz)[(z+h[0][y])%(zsiz+1)]
        sxz_xymat1T[y] = np.transpose(sxz)[(z+h[0][y]+1)%(zsiz+1)]
    sxy = np.transpose(sxz_xymatT) #x-y matrix
    sxy1 = np.transpose(sxz_xymat1T) #x-y matrix
    '''
