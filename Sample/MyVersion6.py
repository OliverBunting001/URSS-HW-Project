...
test=True
if inputs1:
    import inputs1 as inn
if inputs2:
    import inputs2 as inn
if inputs3:
    import inputs3 as inn
if inputs4:
    import inputs4 as inn
if inputs5:
    import inputs5 as inn
if test:
...
g = g / a0 
# Burgers vector: miller indices ->nm
b = b0*a
...
nS = c2s @ n
uS = c2s @ u #definitionally in the y-z plane
#obviously zS = (0,0,1)
...
if abs(cp.dot(u, z)) < eps or abs(cp.dot(u, z)-1) < eps :
    ...
    #w = int((t/un) * cp.linalg.norm(u - uz*z) + 0.5) #not compatible
   # zsiz = int(t/dt + 0.5)
...
def howieWhelan(F_in,Xg,X0i,slocal,alpha,t):
    ...
    G=cp.array([[cp.exp(2*cp.pi*1j*(gamma[0]+1j*q[0])*dt), 0*gamma[0]],
                [0*gamma[0], cp.exp(2*cp.pi*1j*(gamma[1]+1j*q[1])*dt)]])
...
z_vec = (  (cp.linspace(0, zsiz, zsiz+1)  - zsiz/2)*(cp.sin(phi) #ossibly need +0.5???
...
rD_1 = -cp.sin(phi) * z_mat.reshape(xsiz,zsiz+1,1) #xz-matrix (ready for stacking)
rD_2 = cp.cos(phi) * z_mat.reshape(xsiz,zsiz+1,1) #xz-matrix (ready for stacking)
...
if inputs2:
    plt.savefig('inputs2.png')
if inputs3:
    plt.savefig('inputs3.png')
if inputs4:
    plt.savefig('inputs4.png')
if inputs4:
    plt.savefig('inputs5.png')
