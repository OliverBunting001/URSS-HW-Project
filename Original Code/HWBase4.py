...
sxzbulk = np.zeros([xsiz, ysiz, zlen])
plt.imshow(sxz)
Ib, Id = funcs_1.calculate_image(sxz, xsiz, ysiz, zsiz, pix2nm, 
                                 t, dt, s, Xg, X0i, g, b, nS, psi, theta, phi, sxzbulk)
...
space = (np.array([*product(range(xsiz), range(ysiz), range(zlen))])) # all possible triplets of
ax = fig.add_subplot(111, projection='3d')
plt.xlabel("x")
plt.ylabel("y")
ax.set_zlabel("z")
ax.scatter(space[:,0], space[:,1], space[:,2], s=10**-19*sxzbulk**-3)
