...
X0i, X0i2 = inn.X0i, cp.asnumpy(inn.X0i)
Xg, Xg2 = inn.Xg, cp.asnumpy(inn.Xg)
a0, a02 = inn.a0, cp.asnumpy(inn.a0)
nu, nu2 = inn.nu, cp.asnumpy(inn.nu)
t0, t02 = inn.t0, cp.asnumpy(inn.t0)
n, n2 = inn.n, cp.asnumpy(inn.n)
g, g2 = inn.g, cp.asnumpy(inn.g)
s, s2 = inn.s, cp.asnumpy(inn.s)
z, z2 = inn.z, cp.asnumpy(inn.z)
b0, b02 = inn.b0, cp.asnumpy(inn.b0)
u, u2 = inn.u, cp.asnumpy(inn.u)
dt, dt2 = inn.dt, cp.asnumpy(inn.dt)
pix2nm, pix2nm2 = inn.pix2nm, cp.asnumpy(inn.pix2nm)
pad, pad2 = inn.pad, cp.asnumpy(inn.pad)
blursigma, blursigma2 = inn.blursigma, cp.asnumpy(inn.blursigma)
#%%
t, t2 = t0 / pix2nm, t02/pix2nm2
pad, pad2 = pad / pix2nm, pad2/pix2nm2
a, a2 = a0 / pix2nm, a02/pix2nm2
X0i, X0i2 = X0i / pix2nm, X0i2/pix2nm2
Xg, Xg2 = Xg / pix2nm, Xg2/pix2nm2
zlen, zlen2 = int(t/dt + 0.5), int(t2/dt2 + 0.5)
g, g2 = g/a, g2/a2
b, b2 = a*b0, a2*b02
blursigma, blursigma2 = blursigma/pix2nm, blursigma2/pix2nm2
#%%
...
def gdotR(rD, bscrew, bedge, beUnit, bxu, d2c, nu, gD):
    ...
    Rscrew_2 = bscrew*(cp.arctan(rD[...,1].reshape(cp.shape(rX))/rX)-cp.pi*(rX<0))/(2*cp.pi) #component 3 (z) of 3vector xz-matrix
        ...
        for z in range(zsiz+1):
            ...
            rD = np.array((rX,
                           dt*( (0.5+z-zsiz/2)*(np.sin(phi) + xsiz*np.tan(psi)/(2*zsiz)) )
                               + rX*np.tan(theta),
                           0)) / pix2nm#in nm
            gR, printer = gdotR(rD, bscrew, bedge, beUnit, bxu, d2c, nu, gD)
            #if z==0:
            #    test1[1,x] = printer
            rDdz = rD + deltaz
            gRdz, printer = gdotR(rDdz, bscrew, bedge, beUnit, bxu, d2c, nu, gD)
            sxz[x,z] = (gRdz - gR)/dz
...
plt.axis("off")
'''
print(type(inputtest1[0]), inputtest2)
print(inputtest1-inputtest2)
#print(test1)
#print(test2)
#print(test1-test2)
