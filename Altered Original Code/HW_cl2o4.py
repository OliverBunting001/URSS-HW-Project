...
__kernel void propagate_wave(__global float4 *in_out, __global float *sxz,
    ...
    float2 exp_alpha_neg = (float2) {cos(alpha), -sin(alpha)};
    
    float2 big_c[4] = {cos_beta_2, sin_beta_2, -sin_beta_2.x * exp_alpha, cos_beta_2.x * exp_alpha};
    float2 big_c_t[4] = {big_c[0], -sin_beta_2.x * exp_alpha_neg, big_c[1], cos_beta_2.x * exp_alpha_neg};
...
class ClHowieWhelan:
    ...
    def calculate_deviations(self, xsiz, zsiz, pix2nm, dt, u, g, b, c2d, d2c, nu, phi, psi, theta):
        ...
        buf_size = xsiz * (zsiz+1) * 4 #4 bits for each array element
        ...
        # subtract one from the other \begin{pmatrix}4&3&2\\ \:3&4&5\\ \:1&4&7\end{pmatrix}to get the gradient
