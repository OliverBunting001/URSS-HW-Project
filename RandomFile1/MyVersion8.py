...
def HowieWhelan(F_in, ss, XGr, XGi, X0i, t,z): #ss takes the role of slocal for the avoidance of ambiguity
    ...
    #det = cp.exp(2*cp.pi*t * (1j*ss + 1/X0i))[...,None,None]
    #det = cp.tile(det, (1,1,2,1))
...
for zx in tqdm(range(foil_slices)):
    ...
    F = HowieWhelan(F, slocal, Xgr, Xgi, X0i, t)
...
plt.ylabel("y")
ax.set_zlabel("z")
ax.scatter(space[:,0], space[:,1], space[:,2], c=sxzbulk, s=0.01*sxzbulk**-1)
